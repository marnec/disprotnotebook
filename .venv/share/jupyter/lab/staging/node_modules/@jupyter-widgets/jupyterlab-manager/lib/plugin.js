...
import { toArray, filter } from '@phosphor/algorithm';
 ...
 */
function activateWidgetExtension(app, tracker, rendermime, settingRegistry, menu, loggerRegistry) {
    const { commands } = app;
    const bindUnhandledIOPubMessageSignal = (nb) => {
        ...
        if (wManager) {
            wManager.onUnhandledIOPubMessage.connect((sender, msg) => {
                ...
                const data = Object.assign({}, msg.content, { output_type: msg.header.msg_type });
